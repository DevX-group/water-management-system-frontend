...
export const OutstandingBalanceWidget: React.FC = () => {
  ...
  const amount = data?.totalOutstandingAmount ?? 0;
  const billCount = data?.outstandingBills?.length ?? 0;
    ...
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-2">
        ...
        <span className={`text-2xl font-bold ${amount > 0 ? 'text-destructive' : 'text-success'}`}>
          {t('widgetContent.currency')} {Number(amount).toLocaleString()}
        </span>
      </div>
      {amount > 0 ? (
        <span className="text-xs text-muted-foreground flex items-center gap-1">
          <AlertCircle className="w-3 h-3 text-warning" />
          {billCount} {t('widgetContent.unpaidBills')}
        </span>
      ) : (
