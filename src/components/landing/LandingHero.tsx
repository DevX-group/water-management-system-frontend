...
import { Droplet, MoveRight, ChevronRight, Activity } from 'lucide-react';
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-transparent transition-colors duration-500 pt-20">
      ...
      <div className="container mx-auto px-6 relative z-10 flex flex-col items-center text-center">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-accent/50 border border-border mb-10 mt-4"
        >
          <motion.div 
            animate={{ y: [0, -3, 0] }} 
          ...
          <span className={`${isNonEnglish ? 'text-xs sm:text-sm font-semibold' : 'text-sm font-medium'} tracking-wide text-foreground`}>
            {t('hero.badge', 'Hydropay is live now')}
          </span>
        </motion.div>
        ...
        >
          <h1 className={`${
            isNonEnglish
              ? 'text-3xl sm:text-5xl md:text-6xl font-bold leading-[1.25] tracking-normal mb-6'
              : 'text-5xl sm:text-7xl md:text-8xl font-black tracking-[-0.04em] leading-[1.05] mb-8'
          } text-foreground`}>
            {t('hero.titleLine1', 'Fluid tracking.')}<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-br from-blue-600 via-cyan-500 to-teal-400">
              {t('hero.titleLine2', 'Crystal clear.')}
            </span>
          </h1>
          
          <p className={`${
            isNonEnglish ? 'text-base sm:text-lg md:text-xl' : 'text-xl md:text-2xl'
          } text-muted-foreground leading-relaxed mb-10 max-w-2xl mx-auto font-light tracking-normal`}>
            {t('hero.subtitle', 'An immersive platform to monitor every drop, settle bills instantly, and gain unparalleled insight into your utility flow.')}
          </p>
        </motion.div>
        ...
        >
          <Button asChild size="lg" className={`w-full sm:w-auto ${isNonEnglish ? 'h-14 px-8 text-base font-medium' : 'h-16 px-10 text-lg font-semibold'} rounded-full shadow-md bg-blue-600 text-white hover:bg-blue-700 hover:scale-[1.02] transition-all duration-300`}>
            <Link to="/signup">
              {t('hero.diveIn', 'Dive In Now')} <MoveRight className="ml-3 w-5 h-5" />
            </Link>
          </Button>
          <Button asChild variant="outline" size="lg" className={`w-full sm:w-auto ${isNonEnglish ? 'h-14 px-8 text-base font-normal' : 'h-16 px-10 text-lg font-medium'} rounded-full border-border text-foreground hover:bg-accent transition-all duration-300`}>
            <Link to="/login" className="flex items-center">
              {t('hero.enterDashboard', 'Enter Dashboard')} <ChevronRight className="ml-2 w-5 h-5 opacity-50" />
            </Link>
          </Button>
        </motion.div>
             ...
             <div className="bg-background rounded-xl p-8 sm:p-12 border border-border/40 flex flex-col md:flex-row gap-12 items-center min-h-[300px] relative">
                ...
                <div className="flex-[1.5] w-full text-left relative z-10 flex flex-col justify-center">
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-blue-500/10 text-blue-600 dark:text-blue-400 font-medium text-sm mb-6 w-fit">
                    <Activity className="w-4 h-4" /> {t('hero.realtimeMeterSync', 'Real-time Meter Sync')}
                  </div>
                  
                  <div className="grid grid-cols-2 gap-8 mb-8">
                    <div>
                      <p className="text-sm text-muted-foreground font-medium mb-1 uppercase tracking-wider">{t('hero.currentMonth', 'Current Month')}</p>
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground font-medium mb-1 uppercase tracking-wider">{t('hero.estimatedBill', 'Estimated Bill')}</p>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between text-sm text-muted-foreground">
                      <span>{t('hero.safeUsageZone', 'Safe Usage Zone')}</span>
                      <span>{t('hero.avgUsage', '62% of monthly average')}</span>
                    </div>
